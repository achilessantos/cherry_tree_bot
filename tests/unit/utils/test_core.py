...
def mock_camera(mocker):
    mock_camera = mocker.patch("cherry_tree.utils.core.camera")
    return mock_camera
...
def test_get_screenshot_success(mock_camera, mock_cv2_cvt_color):
    # Configura o mock para retornar uma imagem simulada
    fake_image = MagicMock()
    mock_camera.grab.return_value = fake_image
    result = get_screenshot()

    assert result is not None

    mock_camera.grab.assert_called_once()
    mock_cv2_cvt_color.assert_called_once()


def test_get_screenshot_no_image(mocker, mock_camera, mock_cv2_cvt_color):
    mock_camera.grab.return_value = None

    # Define um valor inicial para `latest_screenshot`
    latest_screenshot = mock_cv2_cvt_color
...
def test_get_screenshot_updates_latest(mocker, mock_camera):
    # Configura o mock para retornar uma imagem simulada
    fake_image = MagicMock()
    mock_camera.grab.return_value = fake_image

    # Mocka cv2.cvtColor
    mock_cv2_cvt_color = mocker.patch("cv2.cvtColor", return_value="gray_image")
    ...
    mock_cv2_cvt_color.assert_called_once_with(fake_image, cv2.COLOR_BGRA2GRAY)
...
def test_locate_success(mock_cv2_min_max_loc, mock_cv2_match_template):
    # Simula os retornos do matchTemplate e minMaxLoc
    mock_cv2_match_template.return_value = MagicMock()
    mock_cv2_min_max_loc.return_value = (None, 0.9, None, (50, 50))
    ...
    mock_cv2_match_template.assert_called_once()
    mock_cv2_min_max_loc.assert_called_once()


def test_locate_low_confidence(mock_cv2_min_max_loc, mock_cv2_match_template):
    # Configura uma confiança baixa
    mock_cv2_match_template.return_value = MagicMock()
    mock_cv2_min_max_loc.return_value = (None, 0.7, None, (50, 50))
...
def test_read_image_success(mocker, mock_os_path_exists):
    ...
    assert result is not None
    mock_os_path_exists.assert_called_once_with("valid_path.jpg")


def test_read_image_file_not_found(mock_os_path_exists):
    mock_os_path_exists.return_value = False

    with pytest.raises(FileNotFoundError, match="Image file not found"):
        read_image("invalid_path.jpg")

    mock_os_path_exists.assert_called_once_with("invalid_path.jpg")


def test_read_image_load_failure(mocker, mock_os_path_exists):
    mock_os_path_exists.return_value = True
    ...
    mock_os_path_exists.assert_called_once_with("invalid_image.jpg")
